    ...
    <div className="min-h-full p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-[#64748b] text-sm mt-1">Manage your products and their public pages.</p>
        </div>
        <Link
          href="/projects/new"
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-[88px] rounded-2xl bg-white border border-[#e2e8f0] animate-pulse" />
          ))}
        </div>
      ) : projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 px-6 bg-white rounded-2xl border border-dashed border-[#e2e8f0] text-center">
          ...
          <h2
            className="text-[17px] font-bold text-[#03045e] mb-1"
            style={{ fontFamily: 'var(--font-syne), Syne, sans-serif' }}
          >
            No projects yet
          </h2>
        ...
        <div className="space-y-3">
          {projects.map(project => (
            <div
              key={project.id}
              className="group bg-white rounded-2xl border border-[#e2e8f0] px-4 sm:px-5 py-4 hover:shadow-md transition-shadow duration-150"
            >
              ...
              <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <h3
                    className="font-bold text-[15px] text-[#03045e] truncate"
                    style={{ fontFamily: 'var(--font-syne), Syne, sans-serif' }}
                  >
                    {project.name}
                  </h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[12px] text-[#64748b] font-mono truncate">/{project.slug}</span>
              ...
              <div className="flex flex-wrap items-center gap-1 mt-3 ml-0 sm:ml-14">
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold text-[#475569] hover:bg-[#f1f5f9] transition-colors no-underline"
                >
